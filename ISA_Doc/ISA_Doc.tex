...
\section{Memory Space}
	...
	\subsection*{Stack}
	The stack will only be directly accessible by read\_stack and write\_stack.
	The alloca instruction will be used to allocate stack space and function calls and returns will use the stack to maintain the frame and stack pointers and return address.
	We will enforce stack safety. If any instruction attempts to access stack space that it isn't supposed to be able to access, we will throw and error.
	Any instruction that uses the stack address mode will be able to access the stack section. 

	\subsection*{Heap}
	The heap will be directly accessed by load and store.
...
\section{Program Visible Registers}
	...
	We will also have a stack pointer, frame pointer, program counter.
