...
void mult_connect(void) {
	for(int i = 0; i < BELT_SIZE; i++) {
		belt[i]->connectsTo(mult_reg_bus.IN());
		belt[i]->connectsTo(mult_reg_00_bus.IN());
		for(int j = 0; j < BITS - 1; j++)
			belt[i]->connectsTo(mult_alu_0[j]->OP1());
		belt[i]->connectsTo(mult_reg_4_bus.OUT());
		belt[i]->connectsTo(mult_alu_5.OUT());
		// TODO: carry & overflow
	}
	...
	for(int i = 0; i < BITS/8; i++) {
		...
		mult_reg_3[i]->connectsTo(
			i % 2 == 0 ? mult_alu_4.OP1() : mult_alu_4.OP2());
	}
