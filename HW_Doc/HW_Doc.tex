...
\chapter{Hardware}
	...
	\section{Diagram}
		...
		\begin{figure}[h]
			...
			\begin{tikzpicture}
				...
				\node[draw,rectangle,anchor=west,minimum width=16cm,fill=white] (belt) at (execute1.west |- {0cm, 5cm}) {Belt};
	...
	\begin{figure}[h]
		\centering
		\begin{tikzpicture}
			\node[rectangle,minimum width=8cm,anchor=west] (execute1) at (0cm, 0cm) {};
			\node[rectangle,minimum width=8cm,anchor=west] (execute2) at (execute1.east) {};
			...
			\tikzmath{ let \myscale = 0.5; }
			\foreach \x in {1,2,...,15} {
				\node[ALU, scale=\myscale] (alu 0 \x) at (\myscale * 0cm, \x * \myscale * -3cm) {\scalebox{2.5}{\(<<\)}};
			}
			\foreach \x in {0,1,...,7} {
				\node[ALU, scale=\myscale] (alu 1 \x) at (\myscale * 3cm, \x * \myscale * -6cm + \myscale * -1.5cm) {\scalebox{2.7}{\(+\)}};
			}
			\foreach \x in {0,1,...,3} {
				\node[ALU, scale=\myscale] (alu 2 \x) at (\myscale * 6cm, \x * \myscale * -12cm + \myscale * -4.5cm) {\scalebox{2.7}{\(+\)}};
			}
			\foreach \x in {0,1} {
				\node[ALU, scale=\myscale] (alu 3 \x) at (\myscale * 9cm, \x * \myscale * -24cm + \myscale * -10.5cm) {\scalebox{2.7}{\(+\)}};
			}
			\node[ALU, scale=\myscale] (alu 4) at (\myscale * 12cm, \myscale * -22.5cm) {\scalebox{2.7}{\(+\)}};
		\end{tikzpicture}
